<?php
	...
	$t=mysqli_query($connect,"select voterID from user where voterID='$voterID'");
	...
	if($password==$cpassword){
		...
		$insert=mysqli_query($connect,"insert into user(name,voterID,password,address,photo,role,status,votes,email) values('$name','$voterID','$password','$address','$image','$role',0,0,'$email')");
		if($insert){
			echo '
			<script>
				window.location="../otp/verify.html";
			</script>
			';
		}
		else{
